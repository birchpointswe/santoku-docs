...
return b.count()
...

    {
      ...
      desc = table.concat({
        ...
        "to fetch. That set feeds the fetch option of request, which ",
...
return true
...

    {
      ...
      desc = table.concat({
        ...
        "decoded values (nil for a delete) and the winning clock. ",
        "Writes you make from inside it are NOT captured as local ",
...
return b.title("n1")
...

    {
      ...
      desc = table.concat({
        ...
        "locally, or repairing a row, should not go out to every peer as an ",
        "edit. quiet runs a function with capture suppressed: the write ",
...
return seen[1]
...

    {
      ...
      desc = table.concat({
        ...
        "a hard requirement rather than a precaution. Set it generously, as the ",
        "server API page does: too low and the boot collapses, with create failing ",
        "on its first DDL statement and workers left running without capture. ",
        "sync does not retry, because a timeout that low also breaks the ",
        ...
        "error rather than anything silent. All of this describes boot; once the ",
        "server is serving, busy_timeout stops governing write latency, so set it ",
        "for the backfill and leave it alone.",
      }),
      runnable = false,
      lang = "lua",
      code = [[
