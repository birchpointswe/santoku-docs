return {
  ...
  intro = table.concat({
    ...
    "promises, batched DOM access over a binary command buffer, and a set of ",
    ...
    "driving the DOM through these modules. One rule holds throughout: ",
    "application code never writes JavaScript. The EM_JS shims are inside the ",
...
print(csp.meta(hashes))
...

    {
      ...
      desc = table.concat({
        ...
        "dispatches a db-error CustomEvent carrying detail.error; hook that to show ",
        "a database failure in your own UI.",
      }),
      runnable = false,
      lang = "lua",
      code = [[
...
async(function ()
  ...
  local rows = core.list()       -- becomes an RPC call named "list"
...

    {
      ...
      desc = table.concat({
        "index(opts) renders a complete HTML document. The keys below are the common ",
        "template slots; unknown keys are silently ignored, so a typo shows up as a ",
  ...
  csp = true,
...

    {
      ...
      desc = table.concat({
        ...
        "client code does nothing useful. The options shown below are the ones it ",
        "reads; anything else you pass is ignored silently. precache paths are normalised to ",
